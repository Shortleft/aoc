...
public class Four {
    ...
    public void partOne() {
        List<FourHelper> collect = Arrays.asList(INPUT.split("\n")).stream().map(x -> new FourHelper(x)).collect(Collectors.toList());
        collect.sort(FourHelper::compareTo);
        HashMap<Integer, Integer> map = new HashMap<>();
        HashMap<Integer, HashMap<Integer, Integer>> mapWithMinuteSleepPerGuard = new HashMap<>();
        int currentId = 0;
        LocalDateTime sleep = LocalDateTime.now();
        LocalDateTime wake;
        for(FourHelper c: collect) {
            if (c.getRest().contains("Guard")) {
                currentId = Integer.parseInt(c.getRest().replaceAll("\\D+", ""));
            }
            if(c.getRest().contains("falls asleep")) {
                sleep = c.getTime();
            }
            if(c.getRest().contains("wakes up")) {
                wake = c.getTime();
                if(map.containsKey(currentId)) {
                    Integer currentSleepTime = map.get(currentId);
                    Integer newSleepTime = (int)ChronoUnit.MINUTES.between(sleep, wake);
                    map.replace(currentId, currentSleepTime + newSleepTime);
                } else {
                    map.put(currentId, (int)ChronoUnit.MINUTES.between(sleep, wake));
                }

                int minuteFallAsleep = sleep.getMinute();
                int minuteWokeUp = wake.getMinute();
                HashMap<Integer, Integer> minuteSleep;

                if(mapWithMinuteSleepPerGuard.containsKey(currentId)) {
                    minuteSleep = mapWithMinuteSleepPerGuard.get(currentId);
                } else {
                    minuteSleep =  new HashMap<>();
                }


                for(int i = minuteFallAsleep; i < minuteWokeUp; i++) {
                    if(minuteSleep.containsKey(i)) {
                        Integer minutesSleptSoFarAtTime = minuteSleep.get(i);
                        minuteSleep.put(i, minutesSleptSoFarAtTime + 1);
                    } else {
                        minuteSleep.put(i, 1);
                    }
                }

                mapWithMinuteSleepPerGuard.put(currentId, minuteSleep);
            }
        }
        ...
        HashMap<Integer, Integer> resMap = mapWithMinuteSleepPerGuard.get(key);
        ...
        System.out.println("Key: " + maxEntry2.getKey() + " Value: " + maxEntry2.getValue());

        int maxSleepMinute = 0;

        int maxSleepMinuteFrequency = 0;

        int maxSleepGuardId = 0;

        for(int guardId : mapWithMinuteSleepPerGuard.keySet()) {
            HashMap<Integer, Integer> map1 = mapWithMinuteSleepPerGuard.get(guardId);

            Map.Entry<Integer, Integer> maxEntry3 = null;
            ...
            if(maxEntry3.getValue() > maxSleepMinuteFrequency) {
                maxSleepMinute = maxEntry3.getKey();
                maxSleepMinuteFrequency = maxEntry3.getValue();
                maxSleepGuardId = guardId;
            }

        }

        System.out.println("Guard id: " + maxSleepGuardId + " minute: " + maxSleepMinute + " frequency: " + maxSleepMinuteFrequency + " result: " + maxSleepGuardId * maxSleepMinute);

    }



    private static  String INPUT = "[1518-08-08 00:45] falls asleep\n" +
